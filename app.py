...
if qp_id:
    st.session_state.selected_movie_id = qp_id
...
def poster_grid(cards, cols=6, key_prefix="grid"):
    ...
    for r in range(rows):
        ...
        for c in range(cols):
            ...
            movie_id = m.get("tmdb_id") or m.get("imdb_id")
...
def parse_omdb_search_to_cards(data, keyword: str, limit: int = 24):
    ...
    # OMDB returns dict with 'Search' list
    ...
    for x in final_list[:10]:
        label = f"{x['title']} ({x.get('year','')})" if x.get("year") else x["title"]
...
with st.sidebar:
    ...
    home_category = st.selectbox(
        "Category (for default OMDB seed query)",
...
if st.session_state.view == "home":
    ...
    # HOME FEED MODE (OMDB-search-backed)
    ...
    home_query = home_query_map.get(home_category, "movie")

    home_data, err = api_get_json("/omdb/search", params={"query": home_query, "page": 1})
    if err or home_data is None:
        st.error(f"Home feed failed: {err or 'Unknown error'}")
        st.stop()

    recommendations, cards = parse_omdb_search_to_cards(home_data, home_query, limit=24)
    poster_grid(cards, cols=grid_cols, key_prefix="home_feed")
...
# ==========================================================
    ...
    with right:
        ...
        st.markdown("---")
        st.markdown("### Plot")
        st.write(data.get("plot") or "No plot available.")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if title:
        bundle, err2 = api_get_json(
            "/movie/search",
            params={"query": title, "tfidf_top_n": 12},
        )
        ...
        if not err2 and bundle:
            ...
            st.warning("No TF-IDF recommendations available right now.")
    else:
